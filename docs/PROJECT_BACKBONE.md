...
2026-09-09 当前实施路线：从 v1.1.7 `fa3ccdc` 建立
`fabric-mc1.21.11-tool-set-v1.1.8`；可写副本位于工作区
`toolset-next-worktree`。先在 1.21.8 移除网页地图、修复保留模块并回归，
再迁移 1.21.11 / Loader 0.19.5 / Fabric API 0.141.6+1.21.11。
本轮状态、测试和未验证项以 [实施验收记录](NEXT_IMPLEMENTATION_REVIEW.md) 为准。
以下 v1.1.6 表格仅保留作历史快照，不表示当前发布状态。

| 项目 | 当前事实 |
| --- | --- |
| Minecraft | 1.21.11 (目标；1.21.8 基线已完成回归) |
| Java | 21；已使用项目 JDK `I:\mc smc服\simmc moster hunter 2.0\tools\android-env\jdk-21` 构建 |
| Fabric Loader | 0.19.5 (目标) |
| Fabric API | 0.141.6+1.21.11 (目标) |
| 当前分支 | `fabric-mc1.21.11-tool-set-v1.1.8`（本地、未发布） |
| 候选源码提交 | `09d28e3` 之前的迁移工作树；构建完成后更新 |
...
| 最近验证 | 1.21.8 基线 69/69 单测、完整 build 和隔离客户端回归通过；1.21.11 编译/测试待完成 |
| 真实客户端联调 | 仅使用仓库 `run/client-gametest`；1.21.8 保留模块 UI、输入、Mana 合约和世界切换检查通过；1.21.11 与目标服务器仍未验收 |
...
- 左侧固定导航顺序：总览、奥术 HUD、卷轴计算、饰品配装、发酵与厨具、工具组按键；诊断与日志固定在底部。下一版移除网页地图，不保留地图导航、按钮、快捷键或空位。
- 总览必须说明鼠标入口、快捷键、外置模块接管和当前运行状态。
- 总控和子页面使用中文；长警告、状态和按钮文字必须在不同 GUI 缩放与分辨率下可读、不截断、不重叠。
- 工具组按键进入专属设置页，不跳转 Minecraft 原生控制页；专属页管理四个功能组合子键与诊断子键，显示当前键、修改、恢复和冲突提示。
...
- Action Bar、BossBar 和法杖使用的经验包是状态来源；法杖魔力包仅在目标服、内置模块启用、主手持法杖时与原版经验分离。Mana 显示开关不得改变经验包归属。
...
v1.1.8 不再包含网页地图：删除内部实现、地图专属 Mixin、能力探测、
Xaero/jsoup 依赖、下载器、缓存写入、配置入口、命令、UI 和地图快捷键。
不得自动删除玩家已有的地图配置、缓存、Xaero 数据或外置模组。
旧地图线程/纹理清理问题在新版本中随整个模块移除，不宣称旧 v1.1.7 已修复。
Mod Menu 总入口仍独立于地图存在；缺少 Mod Menu 时不得加载其入口类。

下列条目仅适用于 v1.1.7 及更早版本，已被本次移除决定替代：
...
- 新版本不访问网页地图服务器，不初始化地图 HTTP 客户端、调度器或缓存。保留模块的求解器和状态仍须按生命周期关闭或清空。
...
本节旧版本完成状态属于历史证据；下一版不继承其“通过”结论。
下一版逐项验收和证据见 [实施验收记录](NEXT_IMPLEMENTATION_REVIEW.md)。
...
- 1.21.8 基线回归或目标 1.21.11 / Loader 0.19.5 / API 0.141.6+1.21.11 的完整构建失败。
- 总控或任一子板块鼠标点击、滚轮、拖拽、聊天输入或原生返回被快捷键破坏。
- 外置模块存在时内置模块重复初始化、重复线程、重复事件或重复配置读写。
- 新版本仍注册地图入口、Mixin、快捷键或地图网络任务。
- 诊断自动上传、泄露凭据或写入未说明目录。
- JAR 包含已移除的地图实现、Xaero、jsoup 或错误打包 Mod Menu。
- 用户客户端的手工启动、标题/世界/容器路径和父 Screen 行为没有验收证据。

## 13. 测试清单对照与分配流程

下一版固定顺序与分工：A 删除地图和共享入口中的地图调用；B 修复奥术
HUD 与诊断时区；C 修复卷轴批次容量与布局；D 修复饰品复核及保守发酵/厨具
状态。并发槽位不足时 C/D 可由同一代理顺序执行。主线程负责文件边界审查、
1.21.8 完整回归、1.21.11 API/映射升级、产物审计、文档和记忆同步。
共享文件须有唯一编辑者，禁止代理同时构建同一个目录。
...
### 12.1 v1.1.9 奥术 HUD Mana 闪烁修复记录（2026-09-12）

- v1.1.8 在提交 `10f862c` 中加入法杖签名与 `clearVisualState()`，原意是
  在真正切换法杖时清除旧 Mana、平滑值和动画时间，避免旧法杖状态残留。
- 原实现把 `ItemStack` 对象身份、法术 Lore/名称和动态组件变化纳入签名。
  这会把同一根法杖的右键使用、服务端同步重建以及切换绑定法术误判为换杖。
- 该误判会清空 `wandHeld`、`manaReady`、Mana 数值和动画时间戳；随后新的
  经验包重新启用 HUD，表现为 Mana 条闪烁。实机观察显示，第三槽位右键在
  就绪、Mana 不足、单体 CD 和公共 CD 下均可触发，闪烁次数与左下角反馈消息
  数量相匹配；左键和下蹲键未复现。
- `v1.1.9-dev.20260912.1` 移除了 `ItemStack` 对象身份判断，解决了单纯右键
  尝试释放导致的闪烁。
- `v1.1.9-dev.20260912.2` 进一步将连续性判定收窄为“物品类型 + 选中槽位”，
  不再使用 Lore、法术名称或动态组件。`verifyUnitTests` 通过 71/71，完整构建
  成功；项目所有者在 `simmc-test-desktop` 实机确认奥术 HUD Mana 条闪烁已修复。
- 该结论是针对中间版本 `.2` 的真实客户端验收，不代表已创建正式版本、标签或
  Release。后续若发布，仍需保留真正换槽/离手清理、同一法杖切换法术不闪烁的回归
  测试，并重新核对版本清单。
