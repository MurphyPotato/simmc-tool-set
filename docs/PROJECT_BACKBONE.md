...
- Simes 源码中的 `ArcaneCooldownHud`、`ArcaneStatusHud` 和位置配置是行为
  参考；Mana HUD 明确不属于本项目范围。Tool Set 的生命周期、外置优先和
  配置隔离规则优先于直接复制 Simes 的入口代码。
...
本轮从 v1.1.2 创建 `fabric-mc1.21.8-tool-set-v1.1.3`。P4/P1 使用不重叠 worktree
并行；P3 两个代理在 P4 完成后按文件依赖顺序执行；主线程负责合并、版本、构建、逐条复核
和记忆同步。用户最新消息将地图按钮长短未对齐视为 P2，本轮不单独修复，即使 DOCX 正文
仍把该条显示在 P3 段落中。
