...
public record ArcaneSettings(
    ...
    public ArcaneSettings {
        ...
        searchBudget = searchBudget == null ? SearchBudget.BALANCED : searchBudget;
    }
    public ArcaneSettings(Set<String> excludedMaterials, String selectedRecipe, int quantity,
                          boolean includeMainMaterial, int repeatThreshold) {
        this(excludedMaterials, selectedRecipe, quantity, includeMainMaterial, repeatThreshold, SearchBudget.BALANCED);
    }
    ...
    public ArcaneSettings withExcludedMaterials(Set<String> names) {
        return new ArcaneSettings(names, selectedRecipe, quantity, includeMainMaterial, repeatThreshold, searchBudget);
    }

    public ArcaneSettings withSelectedRecipe(String name) {
        return new ArcaneSettings(excludedMaterials, name, quantity, includeMainMaterial, repeatThreshold, searchBudget);
    }

    public ArcaneSettings withInputs(int nextQuantity, boolean nextIncludeMainMaterial, int nextRepeatThreshold) {
        return new ArcaneSettings(excludedMaterials, selectedRecipe, nextQuantity, nextIncludeMainMaterial, nextRepeatThreshold, searchBudget);
    }

    public ArcaneSettings withSearchBudget(SearchBudget next) {
        return new ArcaneSettings(excludedMaterials, selectedRecipe, quantity, includeMainMaterial, repeatThreshold, next);
    }
}
